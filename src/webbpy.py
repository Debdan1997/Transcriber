...
class hello:
    def GET(self):
    	data = web.input()
    	print(data)
    	return get_mongo_query(data.query)
...
def get_mongo_query(query):
	...
	try:
		...
		return (json.dumps({"result_code":0,"result_type":selection_field,"result":result}))
	except:
		return (json.dumps({"result_code":1,"result":"some error occurred or no result found"}))
